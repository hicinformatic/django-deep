...
class JsonExtract(Func):
    ...
    def __init__(self, json_field, data_path, **extra):
        ...
        if self.vendor not in self.templates:
            raise NotImplementedError(f"JsonExtract n'est pas disponible pour la base {self.vendor}")
        extra['json_field'] = json_field
        extra['data_path'] = getattr(self, f'get_data_{self.vendor}', self.get_data_default)(data_path)
