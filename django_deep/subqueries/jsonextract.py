...
class JsonExtractDate(JsonExtract):
    ...
    templates = {
        ...
        'mysql': "CAST(JSON_UNQUOTE(JSON_EXTRACT(%(json_field)s, '$.%(data_path)s')) AS DATE)",
        'sqlite': "date(json_extract(%(json_field)s, '$.%(data_path)s'))",
        'oracle': "TO_DATE(JSON_VALUE(%(json_field)s, '$.%(data_path)s'), 'YYYY-MM-DD')",
    }


def age_expr_from_date_expr(date_expr):
    """
    Retourne une ExpressionWrapper qui calcule l’âge en années à partir d’une Expression date.
