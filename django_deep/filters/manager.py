...
class DeepFilterManager:
    ...
    This class provides functionality to activate filters based on request parameters,
    ...
    mandatory_filters : list
        A list of mandatory filter names that must be present in the request.

    Methods:
    -------
    activate_filter(request, filter_instance)
        Activates a filter based on request parameters.
    apply_negation(filter_instance)
        Applies negation logic to a filter.
    check_mandatory_filters(request)
        Checks if all mandatory filters are present in the request.
    get_active_filters(request)
        Returns a list of active filters based on the request.
    apply_filters(queryset, request)
    ...
    def activate_filter(self, request, filter_instance):
        """
        ...
        filter_value = request.GET.get(param_name) or request.POST.get(
            param_name
        )
        ...
        if filter_value:
            ...
            activated_filter = filter_instance.__class__(
                field_name=filter_instance.field, value=filter_value
            )
    ...
    def check_mandatory_filters(self, request):
        ...
        for mandatory_filter in self.mandatory_filters:
            ...
            if not (
                request.GET.get(param_name) or request.POST.get(param_name)
            ):
    ...
    def get_active_filters(self, request):
        ...
        self.check_mandatory_filters(request)

        active_filters = []
        for filter_instance in self.filters:
            activated_filter = self.activate_filter(request, filter_instance)
            if activated_filter:
