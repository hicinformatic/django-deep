...
_filter = getattr(settings, 'DEEP_FILTER', ['f', '(', ')'])  # Délimiteurs pour les filtres
_family = getattr(settings, 'DEEP_FAMILY', ['(', ')'])    # Délimiteurs pour les familles de filtres
...
_negative = getattr(settings, 'DEEP_NEGATIVE', '-')  # Préfixe pour indiquer une condition négative
...
_icontains = getattr(settings, 'DEEP_ICONTTAINS', '*')  # Contient sans tenir compte de la casse
_contains = getattr(settings, 'DEEP_CONTAINS', '**')  # Contient avec prise en compte de la casse
