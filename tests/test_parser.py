...
def check_order_by(row, qs, negative=False):
    ...
    test = getattr(DeepParser(queryset=qs, order_enable=True, params={'o': order}).get_queryset().last(), field)
...
def test_queryset(setup_data):
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in reader:
            ...
            if test in globals():
                globals()[test](row, qs)
            test_negative = f'check_{row["test"]}_negative'
            logger.debug(f'Running test: {test_negative}')
            if test_negative in globals():
                globals()[test_negative](row, qs)
